...
parseItems input |> findBottom |> printfn "%s"

let inputChallenge7 = "czlmv (78)
fiwbd (57)
twxnswy (98)
dhrnu (62)
yptrqp (64)
...
parseItems inputChallenge7 |> findBottom |> printfn "%s"

let solve =
    let startItems = parseItems inputChallenge7
    
    let calculated = new Dictionary<string, int>()
    
    let rec loop (items:(string*int*string[])[]) =
        let startCount = calculated.Count
        items 
        |> Array.iter (fun (name, weight, subitems) -> 
                            match Array.isEmpty subitems with
                            | true -> calculated.Add(name, weight) 
                            | false -> 
                                        ...
                                        then
                                            ...
                                            | true -> calculated.Add(name, weight + (weights|>Seq.sum))
                                            | false -> 
                                                    ...
                                                    let correctWeight = weights |> Seq.filter (fun x -> not (x=wrongWeight)) |> Seq.head
                                                    printfn "wrongWeight found: %d" wrongWeight
                                                    printfn "correctWeight found: %d" correctWeight
                                                    let tooMuchWeight = (wrongWeight-correctWeight)
                                                    printfn "too much %d" tooMuchWeight
                                                    printfn "weights: %A" weights
                                                    printfn "subitems: %A" subitems
                                                    printfn "originalItems: %A" (startItems |> Array.filter (fun (name,weight,_) -> subitems |> Seq.contains name))
                                                    printfn "wrong item name: %s" (subitems.[i])
                                                    let (_,w,_) = startItems |> Array.filter (fun (name,_,_) -> name = (subitems.[i])) |> Array.head
                                                    
                                                    printfn "wrong item 's weight: %d" w 
                                                    printfn "result: %d" (w-tooMuchWeight)
                                                    ()
        ...
        let endCount = calculated.Count

        match endCount - startCount with
        | 0 -> calculated
        | _ -> loop (items|> Array.filter (fun (name,_,_) -> not (calculated.ContainsKey name)))

    loop startItems


let calculated = solve |> Seq.toArray
printfn "calculated values: %A, count=%d" calculated (calculated|> Seq.length)
