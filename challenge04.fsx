...
let splitStringBySpaces (s:string) = s.Split([|' '|],StringSplitOptions.RemoveEmptyEntries)

let challenge4 (isValidPhrase:string[]->bool) input =
    input 
    |> splitStringByLines
    |> Seq.sumBy (
        fun row -> if (row |> splitStringBySpaces |> isValidPhrase) then 1 else 0
    )
    
let challenge4a input =
    let isValid (items:string[]) = (items |> Seq.length) = (items |> Seq.distinct |> Seq.length)
    challenge4 isValid input

let sortedString (x:string) = 
    String(x.ToCharArray()|>Array.sort)

let challenge4b input =
    let isValid (items:string[]) = (items |> Seq.length) = (items |> Seq.map sortedString |> Seq.distinct |> Seq.length)
    challenge4 isValid input

printfn "A 'aa bb cc dd ee'=> %d" <| challenge4a "aa bb cc dd ee"
printfn "A 'aa bb cc dd aa'=> %d" <| challenge4a "aa bb cc dd aa"
printfn "A 'aa bb cc dd aaa'=> %d" <| challenge4a "aa bb cc dd aaa"

printfn "B 'abcde fghij'=> %d" <| challenge4b "abcde fghij"
printfn "B 'abcde xyz ecdab'=> %d" <| challenge4b "abcde xyz ecdab"
printfn "B 'a ab abc abd abf abj'=> %d" <| challenge4b "a ab abc abd abf abj"
printfn "B 'iiii oiii ooii oooi oooo'=> %d" <| challenge4b "iiii oiii ooii oooi oooo"
printfn "B 'oiii ioii iioi iiio'=> %d" <| challenge4b "oiii ioii iioi iiio"


let inputChallenge4 = "oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
printfn "Solution 4A, challengeInput => %d" <| challenge4a inputChallenge4
printfn "Solution 4B, challengeInput => %d" <| challenge4b inputChallenge4
