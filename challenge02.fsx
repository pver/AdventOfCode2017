...
let stringsToNumbers = Seq.map System.Int32.Parse
let maxMinDiff (rowNumbers:seq<int>) = ((rowNumbers|>Seq.max) - (rowNumbers|>Seq.min))

let challenge2Checksum (rowCalculation:seq<int>->int) (input:string) = 
    input
    |> splitStringByLines
    |> Seq.sumBy (splitStringByTabs >> stringsToNumbers >> rowCalculation)  

let challenge2aChecksum = challenge2Checksum maxMinDiff
    
let challenge2bChecksum (input:string) = 
    let rec dividablesBySortedRow (numbers:list<int>) =
        let isDividableBy (largest:int) (smallest:int) = (largest % smallest)=0
            
        match numbers with
        | head :: tail ->
            let other = 
                tail 
                |> Seq.map (fun x-> (x, isDividableBy x head))
                |> Seq.filter (fun (_,d) -> d)
                |> Seq.map (fun (x,_) -> x)
                |> Seq.tryHead
            match other with
            | Some x -> (head, x)
            | None -> tail |> dividablesBySortedRow 
        | [] -> failwith "Should not happen"

    let rowCalculation (rowNumbers:seq<int>) = 
        rowNumbers 
        |> Seq.sort 
        |> Seq.toList 
        |> dividablesBySortedRow 
        |> fun (x,y)->y/x

    challenge2Checksum rowCalculation input
    
let inputChallenge2 = "414	382	1515	319	83	1327	116	391	101	749	1388	1046	1427	105	1341	1590
...
printfn "Solution 2A: %d" <| challenge2aChecksum inputChallenge2
printfn "Solution 2B: %d" <| challenge2bChecksum inputChallenge2